...
export default function RetailerInventory() {
  ...
  const totalItems = retailerInventory.length;
  const totalValue = retailerInventory.reduce((sum, item) => sum + (Number(item.quantity) * Number(item.price_per_unit)), 0);
  const lowStock = retailerInventory.filter(item => Number(item.quantity) < Number(item.min_stock_level)).length;
